...
public class RobotContainer {
    ...
    private final double speedDamper = 1;
        ...
        drivetrain.setDefaultCommand(
            ...
            drivetrain.applyRequest(() ->
                drive.withVelocityX((-joystick.getLeftY() * MaxSpeed) / speedDamper) // Drive forward with negative Y (forward)
                    .withVelocityY((-joystick.getLeftX() * MaxSpeed) / speedDamper) // Drive left with negative X (left)
                    .withRotationalRate((-joystick.getRightX() * MaxAngularRate) / speedDamper) // Drive counterclockwise with negative X (left)
            )
        );
